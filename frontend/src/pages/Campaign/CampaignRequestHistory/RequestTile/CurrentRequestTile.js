...
const CurrentRequestTile = ({ request, votersList, donors, campaignId }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (response.status !== 200) {
            setIsError({
                value: true,
                msg: "Cannot add your Vote. Try Again Later",
            });
        } else {
    ...
    return (
        <>
            ...
            <NotificationModal show={showNotificationModal} handleClose={handleCloseNotificationModal} notificationMessage={isError.value ? isError.msg : ("Voted Successfully for the " + request.requestTitle + ". Thank you for your contribution.")} notificationHeader={isError.value ? "Voting Failed" : "Voted Successfully"} />
        </>
    );
}
