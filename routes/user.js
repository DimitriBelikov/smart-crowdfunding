...
router.post("/", upload.any(), (req, res) => {
    const { userName, password, fullName, dob, emailId, currentCity, state } =
        req.body;
    ...
    User.findOne({ emailId })
            ...
            bcrypt.genSalt(parseInt(process.env.SALT), (error, salt) => {
                if (error) {
                    ...
                    return res.status(500).json({
                        msg: "Error while adding user to Database" + error,
                    });
                }
                bcrypt.hash(newUser.password, salt, (error, hash) => {
                    if (error) {
                        ...
                        return res.status(500).json({
                            msg:
                                "Error while adding user to Database: " + error,
                        });
                    }
                    ...
                    newUser
                        ...
                        .then((user) => {
                            console.log(
                                "--> New User Created. Document Saved on Database."
                            );
        ...
        .catch((error) =>
            res
                .status(400)
                .json({ msg: "Error while creating new User: " + error })
        );
});
...
router.get("/:id", (req, res) => {
    User.findById(req.params.id)
        ...
        .catch((error) =>
            res.status(400).json({ msg: "Cannot Find User Details: " + error })
        );
});
            ...
            if (
                ...
                User.findByIdAndUpdate(
                    ...
                    (error, response) => {
                        if (error)
                            res.status(400).json({
                                msg: "Error Updating User Details: " + error,
                            });
...
router.delete("/:id", (req, res) => {
    User.findByIdAndDelete(
        ...
        (error, response) => {
            if (error)
                res.status(400).json({ msg: "Error Deleting User: " + error });
                ...
                .then((isMatch) => {
                    ...
                    jwt.sign(
                        ...
                        (error, token) => {
                            if (error)
                                res.status(400).json({
                                    msg: "Error while creating JWT: " + error,
                                });
                            else {
                                let options = {
                                ...
                                return res
                                    .status(200)
                                    .json({ msg: "Cookie Generated" });
                            }
                        }
                    );
                })
                .catch((error) =>
                    res.status(400).json({ msg: "Error in login: " + error })
                );
        })
        .catch((error) =>
            res.status(400).json({ msg: "Error in login: " + error })
